const AccountingEngine = {
  ...
  saveDailySnapshot(totalRev, date) {
    ...
    const daysInMonth = new Date(targetDateObj.getFullYear(),targetDateObj.getMonth() + 1, 0).getDate();
    const dCost = (r + s + o) / daysInMonth;
    ...
    history[targetDate] = {
      ...
      costSnapshot: {
        rent: (r / daysInMonth).toFixed(2),
        salary: (s / daysInMonth).toFixed(2),
        other: (o / daysInMonth).toFixed(2)
      }
    };
